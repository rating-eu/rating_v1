...
@Component({
    selector: 'jhi-phase-mgm',
    templateUrl: './phase-mgm.component.html'
})
export class PhaseMgmComponent implements OnInit, OnDestroy {
phases: PhaseMgm[];
    currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.phaseService.search({
                query: this.currentSearch,
                }).subscribe(
                    (res: HttpResponse<PhaseMgm[]>) => this.phases = res.body,
                    (res: HttpErrorResponse) => this.onError(res.message)
                );
            return;
       }
        this.phaseService.query().subscribe(
            (res: HttpResponse<PhaseMgm[]>) => {
    ...
    trackId(index: number, item: PhaseMgm) {
        return item.id;
    }

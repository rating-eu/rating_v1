...
public interface SelfAssessmentRepository extends JpaRepository<SelfAssessment, Long> {
    ...
    @Query("select distinct self_assessment from SelfAssessment self_assessment left join fetch self_assessment.companyGroups left join fetch self_assessment.threatagents left join fetch self_assessment.externalaudits left join fetch self_assessment.questionnaires")
    List<SelfAssessment> findAllWithEagerRelationships();

    @Query("select DISTINCT self_assessment from SelfAssessment self_assessment left join fetch self_assessment.companyGroups left join fetch self_assessment.threatagents left join fetch self_assessment.externalaudits left join fetch self_assessment.questionnaires where self_assessment.id =:id")

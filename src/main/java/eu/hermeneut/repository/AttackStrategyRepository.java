...
public interface AttackStrategyRepository extends JpaRepository<AttackStrategy, Long> {
    ...
    @Query("SELECT DISTINCT attack_strategy FROM AttackStrategy attack_strategy INNER JOIN FETCH attack_strategy.levels levels LEFT JOIN FETCH attack_strategy.mitigations WHERE :level MEMBER OF levels")
    List<AttackStrategy> findAllByLevel(@Param("level") Level level);


    @Query("SELECT DISTINCT attack_strategy FROM AttackStrategy attack_strategy INNER JOIN FETCH attack_strategy.phases phases LEFT JOIN FETCH attack_strategy.mitigations WHERE :phase MEMBER OF phases")
    List<AttackStrategy> findAllByPhase(@Param("phase") Phase phase);


    @Query("SELECT DISTINCT attack_strategy FROM AttackStrategy attack_strategy INNER JOIN FETCH attack_strategy.levels levels INNER JOIN FETCH attack_strategy.phases phases LEFT JOIN FETCH attack_strategy.mitigations WHERE :level MEMBER OF levels AND :phase MEMBER OF phases")
